"""T2 parkour AMP task config factories.
...
motion_reference_cfg = MotionReferenceManagerCfg(
    ...
    link_of_interests=[
        ...
        "HipRollRight_Link",
        "HipYawRight_Link",
        "ShankPitchRight_Link",
        "AnkleCrossRight_Link",
        "FootRight_Link",
    ],
    # T2 link_of_interests symmetric augmentation: 32 links
    ...
    symmetric_augmentation_link_mapping=[
        ...
        1,  # HeadYaw_Link
        2,  # HeadPitch_Link
        10,  # AL1_Link <-> AR1_Link
        11,  # AL2_Link <-> AR2_Link
        12,  # AL3_Link <-> AR3_Link
        13,  # AL4_Link <-> AR4_Link
        14,  # AL5_Link <-> AR5_Link
        15,  # AL6_Link <-> AR6_Link
        16,  # AL7_Link <-> AR7_Link
        3,  # AR1_Link <-> AL1_Link
        4,  # AR2_Link <-> AL2_Link
        5,  # AR3_Link <-> AL3_Link
        6,  # AR4_Link <-> AL4_Link
        ...
        18,  # WaistRoll_Link
        19,  # WaistYaw_Link
        26,  # HipPitchLeft_Link <-> HipPitchRight_Link
        27,  # HipRollLeft_Link <-> HipRollRight_Link
        28,  # HipYawLeft_Link <-> HipYawRight_Link
        29,  # ShankPitchLeft_Link <-> ShankPitchRight_Link
        30,  # AnkleCrossLeft_Link <-> AnkleCrossRight_Link
        31,  # FootLeft_Link <-> FootRight_Link
        20,  # HipPitchRight_Link <-> HipPitchLeft_Link
        21,  # HipRollRight_Link <-> HipRollLeft_Link
        22,  # HipYawRight_Link <-> HipYawLeft_Link
        23,  # ShankPitchRight_Link <-> ShankPitchLeft_Link
        24,  # AnkleCrossRight_Link <-> AnkleCrossLeft_Link
        25,  # FootRight_Link <-> FootLeft_Link
    ],
    symmetric_augmentation_joint_mapping=list(T2_symmetric_augmentation_joint_mapping),
    symmetric_augmentation_joint_reverse_buf=list(T2_symmetric_augmentation_joint_reverse_buf),
...
def _parkour_t2_spec() -> mujoco.MjSpec:
    """Build MjSpec for the T2 robot."""
    spec = mujoco.MjSpec.from_file(
        os.path.abspath(T2_MJCF_PATH)
    )
    spec.assets = get_t2_assets(spec.meshdir)
    # Remove the worldbody ground plane - it has condim=1 (no friction) which
    # conflicts with terrain hfield collision. Terrain provides the actual ground.
    ...
    # Parkour only needs feet, shins, and ankles to collide with terrain.
    UPPER_BODY_BODIES = {
        # Head
        "HeadYaw_Link", "HeadPitch_Link",
        # Arms (long chain, problematic with heightfield)
        "AL1_Link", "AL2_Link", "AL3_Link", "AL4_Link", "AL5_Link", "AL6_Link", "AL7_Link",
        "AR1_Link", "AR2_Link", "AR3_Link", "AR4_Link", "AR5_Link", "AR6_Link", "AR7_Link",
        # Hip joints (upper leg, can trigger overflow on sloped terrain)
        "HipPitchLeft_Link", "HipRollLeft_Link", "HipYawLeft_Link",
        "HipPitchRight_Link", "HipRollRight_Link", "HipYawRight_Link",
        # Shank (can trigger overflow on sloped terrain)
        "ShankPitchLeft_Link", "ShankPitchRight_Link",
        # Waist joints
        "WaistPitch_Link", "WaistRoll_Link", "WaistYaw_Link",
    }
    for body in spec.bodies:
        if body.name in UPPER_BODY_BODIES:
            for geom in body.geoms:
                if geom.group == 1:
                    continue  # skip visual geoms (already disabled)
                geom.contype = 0
                geom.conaffinity = 0
...
def instinct_t2_parkour_amp_env_cfg(
    *,
    play: bool = False,
) -> ManagerBasedRlEnvCfg:
    """Build the base T2 parkour AMP environment configuration.
    ...
    robot_cfg = copy.deepcopy(T2_31DOF_CFG)
    robot_cfg.spec_fn = _parkour_t2_spec
    robot_cfg.init_state.pos = (0.0, 0.0, 1.15)

    # Terrain
    terrain_gen = copy.deepcopy(ROUGH_TERRAINS_CFG_PLAY if play else ROUGH_TERRAINS_CFG)
    # Coarser horizontal scale to reduce hfield collision points per geom.
    # T1/G1 use 0.07 (7cm), T2 needs 0.1 (10cm) to avoid overflow with
    # its 32 collision geoms interacting with heightfield terrain.
    terrain_gen.horizontal_scale = 0.1
    ...
    terrain_cfg = InstinctTerrainImporterCfg(
    ...
    if isinstance(joint_pos_action, JointPositionActionCfg):
        joint_pos_action.scale = copy.deepcopy(T2_ACTION_SCALE)

    # Scene visual style
    cfg.scene.spec_fn = _edit_parkour_scene_spec

    # Scene sensors - T2 uses FootLeft_Link and FootRight_Link as feet
    cfg.scene.sensors = (
        ContactSensorCfg(
            name="contact_forces",
        ...
        VolumePointsCfg(
            ...
            entity_name="robot",
            body_names=("FootLeft_Link", "FootRight_Link"),
        ...
        RayCastSensorCfg(
            ...
            frame=ObjRef(type="body", name="FootLeft_Link", entity="robot"),
        ...
        RayCastSensorCfg(
            ...
            frame=ObjRef(type="body", name="FootRight_Link", entity="robot"),
            pattern=GridPatternCfg(resolution=0.12, size=(0.12, 0.0)),
            ray_alignment="yaw",
            max_distance=10.0,
            debug_vis=False,
        ...
        NoisyGroupedRayCasterCameraCfg(
            name="camera",
            frame=ObjRef(type="body", name="WaistYaw_Link", entity="robot"),
            ...
            focal_length=1.0,
            horizontal_aperture=2 * 0.785,
            vertical_aperture=2 * 0.509,
            ray_alignment="yaw",
            offset=NoisyGroupedRayCasterCameraCfg.OffsetCfg(
                # T2 camera at waist yaw, 0.1m forward (x direction), 48deg downward tilt
                pos=(0.1, 0.0, 0.0),
    ...
    cfg.commands = {
        "base_velocity": PoseVelocityCommandCfg(
            ...
            velocity_ranges={
                "perlin_rough": {"lin_vel_x": (0.0, 2.0), "lin_vel_y": (0.0, 0.0), "ang_vel_z": (-1.0, 1.0)},
                "perlin_rough_stand": {"lin_vel_x": (0.0, 0.0), "lin_vel_y": (0.0, 0.0), "ang_vel_z": (0.0, 0.0)},
                "square_gaps": {"lin_vel_x": (0.0, 2.0), "lin_vel_y": (0.0, 0.0), "ang_vel_z": (-1.0, 1.0)},
                "boxes": {"lin_vel_x": (0.0, 2.0), "lin_vel_y": (0.0, 0.0), "ang_vel_z": (-1.0, 1.0)},
                "hf_pyramid_slope_inv": {"lin_vel_x": (0.0, 2.0), "lin_vel_y": (0.0, 0.0), "ang_vel_z": (-1.0, 1.0)},
            },
    ...
    # T2 joint name patterns (underscore suffix: HipPitchLeft_Joint, etc.)
    policy_terms = {
    ...
    # Rewards - T2 specific joint patterns
    cfg.rewards = {
        # ---------- Task rewards ----------
        ...
        "dof_torques_l2": RewardTermCfg(
            ...
            params={
                "asset_cfg": SceneEntityCfg(
                    "robot",
                    joint_names=(".*Hip.*", ".*ShankPitch.*", ".*Ankle.*"),
                )
            },
        ),
        ...
        # T2 freeze upper body: arms (AL1-7, AR1-7) and waist (3 joints)
...
def instinct_t2_parkour_amp_final_cfg(
    *,
    play: bool = False,
) -> ManagerBasedRlEnvCfg:
    """Create the final T2 parkour AMP env configuration.
    ...
    """
    cfg = instinct_t2_parkour_amp_env_cfg(play=play)
