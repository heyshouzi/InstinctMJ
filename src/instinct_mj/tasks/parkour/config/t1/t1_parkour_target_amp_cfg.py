"""T1 parkour AMP task config factories.
...
_PARKOUR_DATASET_DIR = os.path.expanduser("/Workspace/wulv/motion/t1_parkour_motion_reference")
...
def _parkour_t1_spec() -> mujoco.MjSpec:
    """Build MjSpec for the T1 robot."""
    spec = mujoco.MjSpec.from_file(
        os.path.abspath(T1_MJCF_PATH)
    )
...
def instinct_t1_parkour_amp_env_cfg(
    *,
    play: bool = False,
) -> ManagerBasedRlEnvCfg:
    """Build the base T1 parkour AMP environment configuration.
    ...
    robot_cfg = copy.deepcopy(T1_23DOF_CFG)
    robot_cfg.spec_fn = _parkour_t1_spec
    robot_cfg.init_state.pos = (0.0, 0.0, 0.7)
    ...
    if isinstance(joint_pos_action, JointPositionActionCfg):
        joint_pos_action.scale = copy.deepcopy(T1_ACTION_SCALE)

    # Scene visual style
    cfg.scene.spec_fn = _edit_parkour_scene_spec

    # Scene sensors - T1 uses left_foot_link and right_foot_link as feet
    cfg.scene.sensors = (
        ContactSensorCfg(
            name="contact_forces",
        ...
        NoisyGroupedRayCasterCameraCfg(
            name="camera",
            frame=ObjRef(type="body", name="H2", entity="robot"),
    ...
    # T1 joint name patterns (no underscore prefix, different naming convention)
    policy_terms = {
    ...
    # Rewards - T1 specific joint patterns
    cfg.rewards = {
        # ---------- Task rewards ----------
        ...
        "dof_torques_l2": RewardTermCfg(
            ...
            params={
                "asset_cfg": SceneEntityCfg(
                    "robot",
                    joint_names=(".*_Hip_.*", ".*_Knee_Pitch", ".*_Ankle_.*"),
                )
            },
        ),
        ...
        "freeze_upper_body": RewardTermCfg(
            ...
            params={
                "asset_cfg": SceneEntityCfg(
                    "robot",
                    joint_names=(".*_Shoulder_.*", ".*_Elbow_.*", "Waist", "AAHead.*", "Head_pitch"),
                )
            },
        ),
        # ---------- Safety rewards ----------
...
def instinct_t1_parkour_amp_final_cfg(
    *,
    play: bool = False,
) -> ManagerBasedRlEnvCfg:
    """Create the final T1 parkour AMP env configuration.
    ...
    """
    cfg = instinct_t1_parkour_amp_env_cfg(play=play)
